#!/usr/bin/env python3
"""
play_midi.py — Player MIDI CLI con:
 - porta MIDI OUT (mido + python-rtmidi)
 - filtri tracce/canali, seek, tempo-scale, gain
 - visualizzazione opzionale:
     • "grid"  4×4 (pad)
     • "lanes" Beatmania-like (5/7 corsie)
 - METRONOMO / BEAT GRID (tempo map + time signature)
...
Esempi:
    ...
    python play_midi.py --midi song.mid --tracks include:0,2 --channels exclude:9

    # Visualizzazioni
    python play_midi.py --midi song.mid --viz --viz-mode grid  --viz-beats
    python play_midi.py --midi song.mid --viz --viz-mode lanes --viz-lanes 7 --viz-window-seconds 4 --viz-height 22 --viz-beats
"""
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    logger = logging.getLogger("play_midi")
